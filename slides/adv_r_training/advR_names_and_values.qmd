...
This is because R objects are unchangeable, or *immutable*.
Note, the user-side may be unaffected, but performance greatly affected.
For example
```{r}
x[[3]] <- 5
obj_addr(x)
...
Data frames are actually formatted lists (of vectors).
This is best shown visually. Let's look at the textbook.
...
We looked at the characteristics of character vectors.
Now what about their memory?
```{r}
...
What happens in the Environment pane? What happens in memory?
What happens when we remove `x`?
```{r}
rm(x)
# or even
rm(list = ls())
